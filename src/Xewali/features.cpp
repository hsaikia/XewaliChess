...
double Features::eval_influence(const Position& pos)
{
	double influence[2][64];

	for (Color color = WHITE; color <= BLACK; color++)
	{
		for (auto sq = Square::SQ_A1; sq <= Square::SQ_H8; sq++)
		{
			influence[color][sq] = 0.0;
		}
	}

	for (auto sq1 = Square::SQ_A1; sq1 <= Square::SQ_H8; sq1++)
	{
		std::vector<Square> attacks_from_sq1;
		for (auto sq2 = Square::SQ_A1; sq2 <= Square::SQ_H8; sq2++)
		{

			if (pos.piece_attacks_square(sq1, sq2))
			{
				attacks_from_sq1.emplace_back(sq2);
				//if (pos.color_of_piece_on(sq1) == Color::WHITE && pos.color_of_piece_on(sq2) == Color::WHITE)
				//{
				//	std::cout << "White piece on " << square_to_string(sq1) << " defends white piece on " << square_to_string(sq2) << "\n";
				//}
				//else if (pos.color_of_piece_on(sq1) == Color::WHITE && pos.color_of_piece_on(sq2) == Color::BLACK)
				//{
				//	std::cout << "White piece on " << square_to_string(sq1) << " attacks black piece on " << square_to_string(sq2) << "\n";
				//}
				//else if (pos.color_of_piece_on(sq1) == Color::BLACK && pos.color_of_piece_on(sq2) == Color::WHITE)
				//{
				//	std::cout << "Black piece on " << square_to_string(sq1) << " attacks white piece on " << square_to_string(sq2) << "\n";
				//}
				//else if (pos.color_of_piece_on(sq1) == Color::BLACK && pos.color_of_piece_on(sq2) == Color::BLACK)
				//{
				//	std::cout << "Black piece on " << square_to_string(sq1) << " defends black piece on " << square_to_string(sq2) << "\n";
				//}
				//else if (pos.color_of_piece_on(sq1) == Color::WHITE)
				//{
				//	std::cout << "White piece on " << square_to_string(sq1) << " controls empty square on " << square_to_string(sq2) << "\n";
				//}
				//else if (pos.color_of_piece_on(sq1) == Color::BLACK)
				//{
				//	std::cout << "Black piece on " << square_to_string(sq1) << " controls empty square on " << square_to_string(sq2) << "\n";
				//}
			}
		}

		auto n = attacks_from_sq1.size();
		for (const auto& sq : attacks_from_sq1)
		{
			influence[pos.color_of_piece_on(sq1)][sq] += 1.0 / n;
		}
	}

	int influence_score = 0;
	for (auto sq = Square::SQ_A1; sq <= Square::SQ_H8; sq++)
	{
		if (influence[Color::WHITE][sq] > influence[Color::BLACK][sq])
		{
			influence_score++;
			//if (influence[Color::BLACK][sq] > 0)
			//{
			//	std::cout << "Influence on " << square_to_string(sq) << " (W|B) " << influence[Color::WHITE][sq] << ">" << influence[Color::BLACK][sq] << "\n";
			//}
		}
		else if (influence[Color::WHITE][sq] < influence[Color::BLACK][sq])
		{
			influence_score--;
			//if (influence[Color::WHITE][sq] > 0)
			//{
			//	std::cout << "Influence on " << square_to_string(sq) << " (W|B) " << influence[Color::WHITE][sq] << "<" << influence[Color::BLACK][sq] << "\n";
			//}
		}
	...
	//std::cout << "Influence Eval " << influence_score << "\n";

	return influence_score / 64.0;
}

double Features::eval_static(Position& pos)
{
	int res;
	if (has_game_ended(pos, res, false))
	{
		return res;
	}
...
bool Features::has_game_ended(Position& pos, int & result, bool drawCondition)
{
